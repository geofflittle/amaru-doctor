...
use crate::view::flamegraph::render_flamegraph;
...
//         let spans = s.flamegraph_data.load();
...
pub struct OtelFlameGraph;
impl View for OtelFlameGraph {
    fn slot(&self) -> WidgetSlot {
        WidgetSlot::Details
    }
    ...
    fn render(&self, frame: &mut Frame, area: Rect, s: &AppState) -> Result<()> {
        // This will never block the FlamegraphData writer thread
        let data = s.flamegraph_data.load();
        render_flamegraph(frame, area, &data, is_widget_focused(s, self.slot()))
    }
}
